...
from model_utils.test import evaluate_cross, test_cross
from model_utils.plots import plot_auroc
from data.dataset_loader import crossDataset, AddGaussianNoise
...
def train_tuning(params, trial):
    ...
    encoder, bn_dem, decoder = create_model(architecture=params["architecture"], bn_attention=params["bn_attention"], in_channels=params.get("channels", 3))
    encoder = encoder.to(device)
    bn_dem = bn_dem.to(device)
    encoder.eval()
    decoder = decoder.to(device)

    optimizer = get_optimizer(params, list(decoder.parameters()) + list(bn_dem.parameters()))
    ...
    for epoch in range(params["num_epochs"]):
        ...
        if (epoch + 1) % 2 == 0:
            if params["loss_weight_score"]:
                ...
                temp = [1.0, 1.0, 1.0]
            total_auroc, _ = evaluate_cross(encoder, bn_dem, decoder, test_loader, device, weights=params.get("score_weights", [1.0, 0.0]), score_mode=params.get("score_mode", "a"), temp=temp)
...
def train_normal(params, train_loader, test_loader, device):
    ...
    """
    #pre_conv = ChannelReductionCBAM(in_channels=params["channels"], ratio=2, emphasis_channel=0, weight=params["dem_weight"])
    encoder, bn_dem, decoder_dem = create_model(architecture=params["architecture"], bn_attention=params["bn_attention"], in_channels=params.get("channels", 3))
    _, bn_rgb, decoder_rgb = create_model(architecture=params["architecture"], bn_attention=params["bn_attention"], in_channels=params.get("channels", 3))
    encoder = encoder.to(device)
    bn_dem = bn_dem.to(device)
    bn_rgb = bn_rgb.to(device)
    encoder.eval()
    decoder_dem = decoder_dem.to(device)
    decoder_rgb = decoder_rgb.to(device)
    optimizer_dem = get_optimizer(params, list(decoder_dem.parameters()) + list(bn_dem.parameters()))
    optimizer_rgb = get_optimizer(params, list(decoder_rgb.parameters()) + list(bn_rgb.parameters()))
    loss_fn = get_loss_fn(params)
    # lr scheduler
    scheduler_dem = torch.optim.lr_scheduler.StepLR(
        optimizer_dem, 
        step_size=5,
        gamma=params["lr_factor"],
    )
    scheduler_rgb = torch.optim.lr_scheduler.StepLR(
        optimizer_rgb, 
        step_size=5,
        gamma=params["lr_factor"],
    )

    scaler_dem = GradScaler("cuda")
    scaler_rgb = GradScaler("cuda")
    ...
    for epoch in range(params["num_epochs"]):
        ...
        for input in train_data:
            #images = input["image"].to(device)
            dems = input["dem"].to(device)
            rgb = input["rgb"].to(device)
            with autocast(device_type="cuda"):
                inputs_dem = encoder(dems)
                outputs_dem = decoder_dem(bn_dem(inputs_dem))
                loss_dem = loss_fn(inputs_dem, outputs_dem)
            
            optimizer_dem.zero_grad()
            #loss.backward()
            #optimizer.step()
            scaler_dem.scale(loss_dem).backward()
            scaler_dem.step(optimizer_dem)
            scaler_dem.update()
            
            loss_dem_sum += loss_dem.item()
            num_batches += 1

            with autocast(device_type="cuda"):
                inputs_rgb = encoder(rgb)
                outputs_rgb = decoder_rgb(bn_rgb(inputs_rgb))
                loss_rgb = loss_fn(inputs_rgb, outputs_rgb)
        ...
        avg_loss = (loss_dem_sum + loss_rgb_sum) / num_batches
        ...
        if (epoch + 1) % 2 == 0:
            if params["loss_weight_score"]:
                ...
                temp = [1.0, 1.0, 1.0]
            total_auroc, orchard_auroc_dict = evaluate_cross(encoder, bn_dem, bn_rgb, decoder_dem, decoder_rgb, test_loader, device, params["log_path"], temp=temp,
            ...
            if total_auroc > best_auroc:
                ...
                torch.save({'bn_dem': bn_dem.state_dict(), 'decoder_dem': decoder_dem.state_dict(), 'bn_rgb': bn_rgb.state_dict(), 'decoder_rgb': decoder_rgb.state_dict()}, params["model_path"])
            
            scheduler_dem.step()
            scheduler_rgb.step()
    
    test_cross(encoder, bn_dem, bn_rgb, decoder_dem, decoder_rgb, test_loader, device, weights=params.get("score_weights", [1.0, 0.0]), score_mode=params.get("score_mode", "a"), n_plot_per_class=5)
